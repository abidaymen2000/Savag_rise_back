...
from app.services.services_erp.security_admin import hash_password


async def public(admin: AdminInDB) -> AdminPublic:
    return AdminPublic(
        id=str(admin.id),
        email=admin.email,
...
async def validate_permissions(permissions: list[str]) -> list[str]:
    allowed = set(await admin_crud.get_permission_keys())
    unknown = sorted(set(permissions) - allowed)
    if unknown:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, f"Permissions inconnues: {', '.join(unknown)}")
    return sorted(set(permissions))


async def list_admins(page: int, page_size: int, q: Optional[str], is_active: Optional[bool]):
    filters = {}
    if q:
        filters["$or"] = [{"email": {"$regex": q, "$options": "i"}}, {"full_name": {"$regex": q, "$options": "i"}}]
    ...
    return {
        "items": [await public(admin) for admin in items],
        "total": total,
        "page": page,
        "page_size": page_size,
...
async def create_admin(payload):
    existing = await admin_crud.get_by_email(payload.email)
    if existing:
        raise HTTPException(status.HTTP_409_CONFLICT, "Admin deja existant")
    permissions = [] if payload.is_superadmin else await validate_permissions(payload.permissions)
    admin = AdminInDB(
    ...
    return await public(await admin_crud.create(admin))
...
async def get_admin(admin_id: str):
    ...
    return await public(admin)


async def update_admin(admin_id: str, payload, current_super):
    admin = await admin_crud.get_by_id(admin_id)
    if not admin:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Admin introuvable")
    data = payload.model_dump(exclude_unset=True)
    if "permissions" in data:
        data["permissions"] = await validate_permissions(data["permissions"])
    ...
    return await public(await admin_crud.update_admin(admin_id, data))


async def reset_password(admin_id: str, payload):
    admin = await admin_crud.get_by_id(admin_id)
    if not admin:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Admin introuvable")
    return await public(await admin_crud.update_admin(admin_id, {"password_hash": hash_password(payload.new_password)}))


async def delete_admin(admin_id: str, current_super):
    admin = await admin_crud.get_by_id(admin_id)
    if not admin:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Admin introuvable")
